...
def crea_fra_scatter_graph(dfrfracam_os):
	...
	trace_fra = go.Scatter(
		...
		name = 'lines+markers',
		line = dict(shape='spline', color=('#4176A4'), # color azul Binary
					# width=2,
					),
		opacity = 0.8,
	...
	layout = dict(title = 'FRA 1 month IRS CAM off-shore (f1m-os) ',
...
def crea_fra_hist_line(last_val):
	...
	y1 = dfh['1m'].copy()
	y1.iloc[-1] = last_val/100 # divido x 100 porq el val en el dict viene asi para el graf
	...
	trace = go.Scatter(
		...
		# mode = 'lines',
		name='FRA history',
		line=dict(shape='spline', color=('#4176A4'),
				  # width=1,
				  ),
		opacity=0.8,
	)
